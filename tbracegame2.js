$(document).ready(function() {
	...
	$('#begin').on('click', function() {
		$('#fader').slideUp('slow');
			$('#faderIn').fadeIn('slow');
		$('.animals').animate({width:"20em", height:"20em"},
			{queue:false,
				duration:2000})
		$('#begin').fadeOut('slow');
		$('#animalInfo').fadeIn('slow');
	});
	
	$('#results', '#winning').hide();
	
	$('.animals').hover(function() {
		$(this).addClass('newAnimal');
		}, function() {
		$(this).removeClass('newAnimal')
	});
	
	$('.animals').on('click', function(e) {
		var yaClicked = e.target.name;
		$(this).unbind('mouseenter mouseleave');
		$(this).addClass('newAnimal');
		$('#youPicked').append("<p>You've Chosen: </p>" + yaClicked);
		$('#results').hide();
		$('#animalInfo').remove();
		$('#winning').show();
